...
connection_name = "goit_mysql_db_oleh"
...
def generate_delay():
    ...
    time.sleep(25)  # Затримка для перевірки сенсора
...
dag = DAG(
    ...
    description="oleh DAG for medal selection and counting",
...
create_table = MySqlOperator(
    ...
    sql="CREATE TABLE IF NOT EXISTS neo_data.oleh_medals_counts (id INT AUTO_INCREMENT PRIMARY KEY, type VARCHAR(50), count INT, created_at DATETIME);",
...
calc_gold = MySqlOperator(
    task_id="calc_Gold",
    sql="""
        INSERT INTO neo_data.oleh_medals_counts (type, count, created_at)
...
check_for_correctness = SqlSensor(
    ...
    sql="SELECT COUNT(*) FROM neo_data.oleh_medals_counts WHERE created_at >= NOW() - INTERVAL 30 SECOND;",
    mode="poke",
    poke_interval=5,
    timeout=60,
)
